...
uses
  FileCtrl, CommCtrl,
// ABF VCL
  abfConsts, abfVclConsts, abfSysUtils, abfVclUtils, abfTypInfo;


//==============================================================================
// Additional protection routine.
// Date: 08/27/2002
...
{$IfDef D8_ONLY}WN0 = 'Borland Delphi 8';   {$EndIf}
...
begin
{$IfDef abfTrial}
// This application uses a HACKED version of ABF VCL by ABF software, Inc.
// STOP using it! It is ILLEGAL! Do you want to register ABF VCL now?
  Msg :=
    _X('Dxyc0q``|ysqdy~0ecuc0q0XQS[UT0fubcy~0v0QRV0FS\0ri0QRV0cvdgqbu<0Y~s>', 16) + CRLF +
    _X('z}fy	\Z@GN	@]	`]	@Z	`eelnhe	mF	PF\	^HG]	]F	[LN@Z]L[	hko	je	GF^', 41);
  if MessageBoxEx(0, PChar(Msg), PChar(SabfImportant),
    MB_YESNO or MB_ICONSTOP, 0) = IDYES then abfGoToUrlEx(SabfWeb_Register, True);
  Halt;
{$EndIf abfTrial}
end;
...
begin
{$IfDef abfTrial}
  if not abfIsDelphiRunning then _TADA;
{$EndIf abfTrial}
end;
