...
// asynchronously to the main VCL thread.
  ...
  // Properties Get/Set
    ...
    procedure SetSrcName(const A: string); override;
    procedure SetDstName(const A: string); override;
  public
...
// Add FileName 
  if FileName <> '' then
    Result := abfAddChar(Result, SabfInet_Delimiter_Slash) + FileName;
// Add ExtraInfo 
  if ExtraInfo <> '' then
...
constructor TabfCustomInternetCopyThread.Create(const ASrcName, ADstName: string;
...
procedure TabfCustomInternetCopyThread.SetSrcName(const A: string);
begin
...
procedure TabfCustomInternetCopyThread.SetDstName(const A: string);
begin
